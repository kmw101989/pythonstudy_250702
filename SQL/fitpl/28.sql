/* ╔══════════════════════════════════════════════════════════════════╗
   ║  FitPl 알고리즘 DDL (최종 출력 = user_id + products 모든 컬럼)   ║
   ║  - 방법②: 실행 시 user_id=@uid 필터                               ║
   ║  - 수정안 1~4 반영 + strict/relaxed 보강                          ║
   ╚══════════════════════════════════════════════════════════════════╝ */
USE fitpl;
...
/* ───────────────────────────────────────────────────────────────────
   [10] 제품 정규화·버킷·후보 (수정안1: URL 기준 dedup + 모든 컬럼 보존)
   ─────────────────────────────────────────────────────────────────── */
...
-- 모든 products 컬럼 보존 + URL 기준 대표 1행 선택
CREATE OR REPLACE VIEW v_products_norm AS
WITH ranked AS (
  SELECT
    p.brand, p.main_category, p.color, p.discount_rate, p.gender_en, p.img_url,
    p.material, p.monthly_views, p.price, p.product_id, p.product_name, p.product_name_detail,
    ...
    ROW_NUMBER() OVER (PARTITION BY p.product_url ORDER BY p.product_id DESC) AS rn
  FROM products p
  WHERE p.product_url IS NOT NULL
)
SELECT
  brand, main_category, color, discount_rate, gender_en, img_url,
  material, monthly_views, price, product_id, product_name, product_name_detail,
...
strict AS (  -- 기존 후보
  SELECT
    ...
    upc.material, upc.monthly_views, upc.price, upc.product_id, upc.product_name, upc.product_name_detail,
...
SELECT
  ...
  r.material, r.monthly_views, r.price, r.product_id, r.product_name, r.product_name_detail,
...
/* ───────────────────────────────────────────────────────────────────
   [12] Top20 (strict + relaxed)  — user_id + products 모든 컬럼
   ─────────────────────────────────────────────────────────────────── */
