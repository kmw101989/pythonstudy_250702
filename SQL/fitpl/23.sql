...
FROM users WHERE user_id=2 AND activity_tag_1 IS NOT NULL AND TRIM(activity_tag_1) <> ''
UNION ALL
SELECT user_id,
       REGEXP_REPLACE(REGEXP_REPLACE(LOWER(TRIM(activity_tag_2)),'\\s+',''), '[-_]', '')
FROM users WHERE user_id=2 AND activity_tag_2 IS NOT NULL AND TRIM(activity_tag_2) <> ''
UNION ALL
SELECT user_id,
       REGEXP_REPLACE(REGEXP_REPLACE(LOWER(TRIM(activity_tag_3)),'\\s+',''), '[-_]', '')
FROM users WHERE user_id=2 AND activity_tag_3 IS NOT NULL AND TRIM(activity_tag_3) <> '';
...
FROM (SELECT 2 AS user_id) u CROSS JOIN category c;
...
UNION ALL
SELECT 
  utk.user_id, m2.category_id, AVG(m2.w_eq) AS climate_cat_w
FROM v_user_trip_key utk
JOIN (
...
JOIN v_climate_cat_equalweight m2
  ON m2.climate_bucket_norm = rbm2.bucket_norm_norm
WHERE 
  NOT EXISTS (
    SELECT 1 FROM v_region_climate_buckets_norm rb2
    WHERE (rb2.region_id, rb2.month) = (SELECT region_id, month FROM v_user_trip_key)
  )
